...
struct DisplayServicesBridge {
    ...
    /// Note: the framework also exports `DisplayServicesSetBrightnessSmooth`,
    /// which sounds like the right call for avoiding visible stepping. It
    /// isn't usable here: on this hardware it returns success and changes
    /// nothing, so relying on its return value silently breaks brightness.
    @discardableResult
