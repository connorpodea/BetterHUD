...
final class VolumeFeedbackSound {
    ...
    /// Plays the click, honoring the system setting.
    ///
    /// Holding Shift inverts the setting for that press, matching how the
    /// native keys behave.
    func play(shiftHeld: Bool) {
        guard isEnabledSystemWide != shiftHeld, let sound else { return }
